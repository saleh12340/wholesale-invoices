...
function loadCustomers(): CustomerAccount[] {
  try { const parsed = JSON.parse(localStorage.getItem(CUSTOMERS_KEY) || '[]'); return Array.isArray(parsed) ? parsed : []; }
  catch { return []; }
}
...
async function savePng(button: HTMLElement) {
  ...
  const canvas = renderReceiptToCanvas(invoice, { storeName:settings.storeName, storeSubtitle:settings.storeSubtitle, storePhone:settings.storePhone, currency:settings.currency, thermalWidth:settings.thermalWidth });
...
function hideOtherPrinters() {
  ...
  document.querySelectorAll('button').forEach((button) => {
    ...
    if (unwanted.some((label) => text.includes(label))) {
      (button as HTMLElement).style.display = 'none';
      (button as HTMLElement).setAttribute('aria-hidden', 'true');
    }
  });
}
export function startExportAndShareEnhancer() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (label.includes('واتساب (صورة)')) {
      ...
      void shareWhatsAppImage(button).catch((e) => { console.error(e); window.alert('تعذر تجهيز صورة الفاتورة لواتساب. تأكد من وجود الفاتورة ورقم العميل.'); });
      return;
    }
    if (label.includes('حفظ PNG')) {
      event.preventDefault(); event.stopImmediatePropagation();
      void savePng(button).catch((e) => { console.error(e); window.alert('تعذر حفظ صورة PNG في مجلد Downloads.'); });
    }
  }, true);
  hideOtherPrinters();
  window.setInterval(hideOtherPrinters, 1000);
}
