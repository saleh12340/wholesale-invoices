...
function getInvoices(name: string): Invoice[] {
...
function status(invoice: Invoice): string {
...
function renderRows(container: HTMLElement, invoices: Invoice[]) {
  const signature = invoices.map((i) => `${i.id}:${i.total}:${i.paymentType}:${i.date}`).join('|');
  if (container.dataset.signature === signature) return;
  container.dataset.signature = signature;
  container.replaceChildren();
  if (!invoices.length) {
    const empty = document.createElement('div');
    empty.className = 'text-[10px] text-slate-400 py-2';
    empty.textContent = 'لا توجد فواتير مرتبطة بهذا العميل';
  ...
  invoices.slice(0, 30).forEach((invoice) => {
    ...
    no.className = 'font-mono text-[10px] font-bold';
    no.textContent = `#${invoice.number}`;
    const info = document.createElement('div');
    info.className = 'min-w-0';
    const date = document.createElement('div');
    date.className = 'text-[10px] truncate';
    date.textContent = `${invoice.date || '-'} ${invoice.time || ''}`.trim();
    const state = document.createElement('div');
    state.className = `text-[9px] font-bold ${invoice.paymentType === 'credit' ? 'text-amber-600' : 'text-emerald-600'}`;
    state.textContent = status(invoice);
    info.append(date, state);
    const total = document.createElement('span');
    total.className = 'font-mono text-[10px] font-black whitespace-nowrap';
    total.textContent = amount(invoice.total);
    row.append(no, info, total);
    container.appendChild(row);
  });
}

function addInvoiceSection(parent: HTMLElement, invoices: Invoice[], modal = false) {
  const section = document.createElement('div');
  section.className = modal
    ? 'mx-4 mb-2 p-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900/60'
    : 'mt-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700/70';
  section.setAttribute(modal ? MODAL_MARKER : CARD_MARKER, 'true');
  const title = document.createElement('div');
  title.className = 'text-[11px] font-black mb-1.5';
  title.textContent = modal ? 'فواتير العميل المرتبطة' : 'فواتير العميل';
  const count = document.createElement('span');
  count.className = 'float-left text-[9px] font-bold text-slate-400';
  count.textContent = `${invoices.length} فاتورة`;
  title.appendChild(count);
  const rows = document.createElement('div');
  rows.dataset.invoiceRows = 'true';
  section.append(title, rows);
  renderRows(rows, invoices);
  ...
  headings.forEach((heading) => {
    ...
    if (text.startsWith('كشف حساب:')) {
      ...
      if (!section) {
        ...
        rows.dataset.invoiceRows = 'true';
        section.append(title, rows);
        const banner = Array.from(modal.querySelectorAll('.p-4')).find((el) => (el.textContent || '').includes('الرصيد المتبقي الإجمالي')) as HTMLElement | undefined;
        if (banner) banner.insertAdjacentElement('afterend', section); else modal.appendChild(section);
      }
      const rows = section.querySelector('[data-invoice-rows]') as HTMLElement | null;
      if (rows) renderRows(rows, invoices);
      return;
    }
    const card = heading.closest('div.bg-white') as HTMLElement | null;
    if (!card) return;
    if (card.querySelector(`[${CARD_MARKER}]`)) return;
    addInvoiceSection(card, getInvoices(text));
  });
}
...
function installWhatsApp() {
  document.addEventListener('click', (event) => {
    ...
    if (!button || !(button.textContent || '').includes('واتساب')) return;
...
export function startCustomerInvoicesEnhancer() {
  ...
  window.setInterval(enhance, 1500);
  installWhatsApp();
}
